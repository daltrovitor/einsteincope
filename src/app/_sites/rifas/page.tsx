...
export default async function RifasLandingPage() {
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {raffles?.map((raffle, index) => (
              <div key={raffle.id} className="bg-[#F8F5EE] rounded-3xl overflow-hidden shadow-lg border-2 border-[#8E5A3C]/20 hover:border-[#8E5A3C] transition-all hover:-translate-y-2 duration-300 flex flex-col group">
                ...
                <div className="p-8 flex flex-col flex-1 relative z-20 bg-[#F8F5EE]">
                  ...
                  <h3 className="text-3xl font-black text-[#4A2B1D] mb-2">{raffle.title}</h3>
                  <p className="text-[#8E5A3C] mb-6 font-medium line-clamp-2">{raffle.description}</p>
                  <div className="mt-auto">
                    <Link href={`/${raffle.slug}`} className="block w-full bg-[#4A2B1D] text-white text-center py-4 rounded-2xl font-bold text-xl hover:bg-[#3A2217] transition-all shadow-lg hover:shadow-xl hover:scale-[1.02] active:scale-[0.98]">
                      Comprar Rifa
                    </Link>
                  </div>
                </div>
